...
class GraphPreprocessor():
    ...
    def add_split(self, filepath, name):
        ...
        self.splits[name] = {'sentences': sentences, 'sentences_dependency_triples': sentences_dependency_triples, 'sentences_labels': sentences_labels}
...
def load_data(A, X, Y, split_name):
    ...
    split_y = [to_categorical(y.toarray()[0], num_classes=9) for y in Y[split_name]]
...
def batch_generator(A, X, Y, split_name, batch_size=16):
    ...
    while True:
        ...
        split_y = np.array([to_categorical(y.toarray()[0], num_classes=9) for y in Y[split_name][batch_start:batch_end]])
